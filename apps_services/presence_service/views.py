...
def registrar_presenca(request):
    ...
    if request.method == "POST":
        ...
        # Busca a aula e extrai o IP do cliente [cite: 98]
        aula = get_object_or_404(Aula, id=aula_id)
        ip_cliente = request.META.get('REMOTE_ADDR')

        # 1. Validar autenticação do aluno [cite: 87, 90]
        ...
        # 2. Validar Token do QR Code [cite: 80, 82]
        if str(aula.token_qr) != token:
            return render(request, 'erro.html', {'msg': 'Token de segurança inválido ou expirado.'})

        # 3. Validar se o aluno já registrou presença (Uso de Cache Redis) [cite: 91, 169, 173]
        ...
        # 4. Validar Horário da Aula [cite: 92]
        ...
        # 5. Validar Rede da Universidade (IP/NAT) [cite: 94, 97, 99]
        ...
        # 6. Validar Geolocalização (Raio permitido da sala) [cite: 93, 101, 104]
        ...
        if distancia > aula.sala.raio_permitido: # Raio padrão sugerido: 50m [cite: 106]
            return render(request, 'erro.html', {'msg': f'Você está fora do raio da sala ({int(distancia)}m).'})

        # 7. Marcar no Cache Redis antes de enviar para a fila (Evita concorrência) [cite: 170, 174]
        # Expira em 2 horas para garantir que o aluno não registre novamente na mesma aula
        cache.set(cache_key, True, timeout=7200)

        # 8. Enviar evento para Fila (RabbitMQ) para gravação assíncrona no banco [cite: 176, 178, 182]
    ...
    # Caso seja um GET (Acesso inicial via link do QR Code) [cite: 83, 86]
    context = {
